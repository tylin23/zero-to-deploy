...
const POS = ["好", "棒", "讚", "愛", "喜歡", "開心", "優", "推", "great", "good", "love", "happy", "nice", "讚讚"];
const NEG = ["爛", "差", "討厭", "難過", "糟", "失望", "生氣", "雷", "bad", "hate", "sad", "angry", "terrible"];
...
function analyzeSentiment(text) {
  ...
  if (pos >= neg) return { label: "正面", emoji: "😊", score: Math.min(0.99, 0.6 + pos * 0.12), color: "var(--success)" };
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="callout callout-info">
        <b className="text-ink">Hugging Face</b> 是 AI 界的大本營，放了很多現成的模型。它的 <b className="text-ink">Spaces</b> 讓你把一個互動式 AI App（用 Gradio / Streamlit 寫）<b className="text-ink">一鍵部署</b>、直接拿到公開網址 —— 而且不用自己準備昂貴的 GPU 伺服器。
      </div>
    ...
    <div className="space-y-4">
      ...
      {phase === "create" && (
        <Browser url="huggingface.co/new-space">
          ...
          <div className="inline-flex items-center gap-1.5 text-[12.5px] font-extrabold text-accentDark bg-accentSoft py-1 px-3 rounded-full mb-2.5">👉 按「Create Space」，平台就會幫你建置</div>
          <div><button type="button" className="gh-btn gh-btn-green animate-pulseRing" onClick={create}>Create Space</button></div>
        </Browser>
      )}
      ...
      {result && (
        <div className="mt-3 bg-surface border border-line rounded-[12px] p-3.5 animate-pop">
          <div className="flex items-center gap-2">
            ...
            <div>
              <div className="font-extrabold text-ink" style={{ color: result.color }}>{result.label}</div>
          ...
          <div className="mt-2 h-2 rounded-full bg-surface2 overflow-hidden">
            <div className="h-full rounded-full" style={{ width: `${result.score * 100}%`, background: result.color }} />
          </div>
        </div>
      )}
...
function RealStep({ onFinish }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="callout">
        最快的方式：找一個現成的 Space 直接玩，或按「Duplicate this Space」複製一份成自己的。之後想自己做，就建一個 Gradio Space、把 <code className="font-mono text-sm bg-surface2 px-1.5 py-0.5 rounded border border-line">app.py</code> 推上去即可。
      </div>

      <div className="grid gap-2.5">
        <a href="https://huggingface.co/spaces" target="_blank" rel="noopener noreferrer" onClick={() => setOpened(true)}
          className="gh-btn justify-between !py-3 !text-[15px]"><span>🔗 逛逛 Hugging Face Spaces</span><span>↗</span></a>
        <a href="https://huggingface.co/new-space" target="_blank" rel="noopener noreferrer" onClick={() => setOpened(true)}
          className="gh-btn justify-between !py-3 !text-[15px]"><span>🔗 建立我的第一個 Space</span><span>↗</span></a>
      </div>
      {opened && <p className="text-success text-sm font-bold">✓ 開好了！挑一個 Space 玩玩看，或按 Duplicate 複製成自己的。</p>}
