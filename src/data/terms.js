  ...
  },
  {
    ...
    more: {
      ...
      items: [
        [
          "換一個有開放的資料源",
          "最省事。有些資料集開放、有些沒開，開不開是對方決定的。第 4 關用的 YouBike 即時資訊就有開放，所以網頁直接 fetch 得到。",
        ],
        [
          "讓「伺服器」去抓，不要讓瀏覽器去抓",
          "CORS 只管瀏覽器。所以換一台不是瀏覽器的機器去抓就好：排一個每天（或每月）自動執行的工作，去把資料抓回來、存成你自己網站裡的一個 .json 檔，網頁再讀自己家的檔案 —— 同一個來源，當然不會被擋。代價是資料變成「定時更新的快照」，不是即時的；好處是資料源掛掉時你的網頁還活著。這就是第 5 關「定時觸發」在做的事。",
        ],
        [
          "請資料提供方開放",
