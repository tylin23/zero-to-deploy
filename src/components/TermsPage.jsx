  ...
  return (
    <div>
      <div className="text-center mb-5">
        ...
        <p className="text-muted text-sm">給剛接觸開發的你：不用會實作，但懂了這些「為什麼要這樣設計」，做事更順、也更能跟資訊單位溝通。</p>
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-4 items-start">
        {shown.map((t) => (
          <TermCard key={t.id} t={t} isOpen={open.has(t.id)} onToggle={() => toggle(t.id)} navigate={navigate}
      ...
      <button type="button" onClick={onToggle} className="w-full text-left p-4 sm:p-5 flex gap-3 items-start">
        ...
        <div className="flex-1">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <span className="ml-auto text-xs font-bold px-2 py-0.5 rounded-full" style={{ background: "color-mix(in srgb, " + cat.color + " 16%, var(--surface))", color: "var(--ink)" }}>{cat.label}</span>
          </div>
        ...
        <div className="px-4 sm:px-5 pb-5 pt-0 space-y-3 border-t-2 border-line">
          ...
          {(t.goLevel || t.related) && (
            <div className="flex flex-wrap gap-2 pt-1">
              {t.goLevel && (
                <button type="button" className="gh-btn !py-2" onClick={() => navigate("#/level/" + t.goLevel)}>
                  🎮 去玩「{levelMeta(t.goLevel).title}」互動關 →
                </button>
              )}
              {(t.related || []).map((lid) => (
                <button key={lid} type="button" className="gh-btn !py-2" onClick={() => navigate("#/level/" + lid)}>
...
function CacheDemo() {
  ...
  const query = async () => {
    ...
    if (cached) { setFrom("cache"); setStatus("done"); setSaved((s) => s + 1); return; }
    setStatus("loading"); setFrom(null);
    await sleep(1100);
    setCached(true); setFrom("source"); setStatus("done");
  };
  const invalidate = () => { setCached(false); setFrom(null); setStatus("idle"); };

  return (
    <div className="border-2 border-line rounded-[14px] bg-surface2 p-3.5">
      <div className="text-xs font-extrabold text-muted mb-2">🧪 玩玩看：連按幾次「查詢」，再按「資料更新了」</div>
      <div className="flex flex-wrap gap-2 items-center">
        <button type="button" className="btn btn-accent !py-2 !px-4 !text-sm" onClick={query} disabled={status === "loading"}>🔍 查詢空氣品質</button>
        <button type="button" className="gh-btn !py-2" onClick={invalidate}>🔄 資料更新了（清快取）</button>
        <span className="text-xs font-bold px-2 py-1 rounded-full" style={{ background: cached ? "var(--success-soft)" : "var(--surface)", color: cached ? "var(--success)" : "var(--muted)", border: "1px solid var(--border)" }}>
          快取：{cached ? "已存有" : "空的"}
        </span>
      </div>
      <div className="mt-2 text-sm font-bold min-h-[1.4em]">
        {status === "loading" && <span className="text-muted">⏳ 去資料源抓…（約 1.2 秒）</span>}
        {status === "done" && from === "source" && <span style={{ color: "var(--danger)" }}>🐢 這次從資料源抓回（慢）</span>}
        {status === "done" && from === "cache" && <span style={{ color: "var(--success)" }}>⚡ 從快取直接回（幾乎 0 秒）</span>}
      </div>
...
function IdemDemo() {
  ...
  const setMode = (v) => { setIdem(v); setCases([]); };
  const submit = () => {
    setCases((cs) => {
      if (idem) {
  ...
  return (
    <div className="border-2 border-line rounded-[14px] bg-surface2 p-3.5">
      ...
      <label className="flex items-center gap-2 mb-2.5 text-sm font-bold cursor-pointer">
        <input type="checkbox" checked={idem} onChange={(e) => setMode(e.target.checked)} style={{ width: 20, height: 20, accentColor: "var(--mint)" }} />
        開啟冪等（用「申請編號」判斷是不是同一筆）
      </label>
      <button type="button" className="btn btn-accent !py-2 !px-4 !text-sm" onClick={submit}>📨 民眾送出申請</button>

      <div className="mt-2.5 grid gap-1.5">
        {cases.length === 0 ? (
          <div className="text-muted text-sm">還沒有案件——連按上面的按鈕看看</div>
        ) : cases.map((c) => (
          <div key={c.id} className={`text-sm px-3 py-1.5 rounded-lg border-2 ${c.dup ? "border-danger bg-dangerSoft text-danger font-bold" : "border-line bg-surface text-ink"}`}>
      ...
      {cases.length > 0 && (
        <div className="text-xs mt-2 font-bold" style={{ color: dups ? "var(--danger)" : "var(--success)" }}>
          {idem ? "✅ 不管按幾次，都只建立 1 筆案件" : dups ? `❌ 建立了 ${cases.length} 筆重複案件！` : "按第二次看看會怎樣…"}
        </div>
      )}
    </div>
  );
