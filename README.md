...
| 關卡                     | 主題                     | 互動設計重點                                                                      |
| ------------------------ | ------------------------ | --------------------------------------------------------------------------------- |
| 9 · Windows / Linux 自架 | **內網自用 vs 對外**     | 三個開關 + 分別測「同仁從市府內網連」與「市民從外網連」，帶出對外需資安評估與核准 |
| 10 · Docker              | 把 App 和環境一起打包    | 選擇是否打包環境 → 在別台機器 run，體會可攜性                                     |
| 11 · EXE 執行檔          | **不上網的那一種「給別人用」** | 四題「做網頁還是做 EXE」的判斷 → 打包示意 ＋ **防毒誤判**（機關現場第一道牆）→ **怎麼發新版本**：版本號 `v1.4.2` 怎麼看、changelog 六種分類（新增／變更／修正／移除／即將移除／資安）、三組「寫給程式看 vs 寫給使用者看」的改寫對照、可複製範本，最後帶出「網頁改一次所有人就是新的，EXE 的更新要自己扛」 |
...
關卡頁另有麵包屑（第 N / 11 關、所屬階段）與底部的上一關／下一關。
...
`tests/e2e.mjs` 會把 **11 關全部走完一遍**（含每關的互動、測驗與過關），並檢查「回上一步」與進度是否到 100%。用來確認改動沒有弄壞任何一關。
