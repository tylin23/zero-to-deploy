  ...
  return (
    <div className="space-y-4">
      ...
      <div className="callout callout-info">
        有沒有遇過「在我電腦可以跑，在你那邊卻壞掉」？多半是因為兩台電腦的
        <b className="text-ink">環境不一樣</b>（版本、套件、設定）。<b className="text-ink">Docker</b> 把 App
        和它需要的整套環境一起裝進一個<b className="text-ink">容器（container）</b>
        ，就像貨櫃：不管搬到哪艘船，裡面都一模一樣。行政上很適合把一個
        <b className="text-ink">局處內部小系統（例如月報表產生器、陳情案件統計）</b>
        打包，讓不同同仁的電腦或機關伺服器都跑出一致結果。
      </div>
...
function BuildStep({ onNext }) {
  ...
  const run = () => {
    if (includeEnv) {
      setRunResult({
        ok: true,
        text: "✅ 在兩台機器上都跑起來了，而且結果一模一樣！因為環境被一起打包在 image 裡。",
      });
      setWin(true);
    } else {
      setRunResult({
        ok: false,
        text: "❌ 同事那台乾淨電腦沒裝 Node，容器裡也沒有 → 跑不起來。這就是「在我電腦可以跑、你那邊壞掉」。把環境一起打包再試一次！",
      });
    }
  };
    ...
    <div className="space-y-4">
      ...
      <div className="border-2 border-line rounded-[18px] bg-surface2 p-4 space-y-2">
        ...
        <div className="flex items-center gap-2 text-sm">
          <span className="text-success">✓</span> 你的程式碼{" "}
          <code className="font-mono text-xs bg-surface px-1.5 py-0.5 rounded border border-line">
            server.js
          </code>
        </div>
        <label
          className="flex items-center gap-2.5 py-2.5 px-3 border-2 rounded-xl cursor-pointer bg-surface transition-colors"
      ...
      {phase === "built" && (
        <>
          ...
          {runResult && (
            <div
              className={`py-3 px-4 rounded-[14px] text-sm font-bold ${runResult.ok ? "bg-successSoft text-success" : "bg-dangerSoft text-danger"}`}
            >
              {runResult.text}
            </div>
          )}
    ...
    <div className="space-y-4">
      ...
      <div>
        ...
        <pre className="font-mono text-[13px] bg-surface2 border border-line rounded-[10px] p-3.5 overflow-x-auto whitespace-pre">
          {
            "$ docker build -t my-app:1.0 .   # 打包成 image\n$ docker run -p 3000:3000 my-app:1.0  # 把容器跑起來"
          }
        </pre>
      </div>
