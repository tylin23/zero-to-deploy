...
export default function TopBar({ navigate }) {
  ...
  return (
    <header
      className="sticky top-0 z-50 flex items-center justify-between gap-3 py-3 px-4 sm:px-10 border-b border-line backdrop-blur-md"
      style={{ background: "color-mix(in srgb, var(--surface) 78%, transparent)" }}
    >
      <button
        type="button"
        onClick={() => navigate("#/")}
        className="flex items-center gap-2.5 text-ink font-extrabold"
        aria-label="回首頁"
      >
