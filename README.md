...
| 5 · API 基礎                 | 前後端怎麼對話                                   | 餐廳點餐比喻 + 站內 API 測試器（送出一次，看回來的 JSON；點出「這支 API 一次給你全部，要篩要自己篩」）+ **怎麼看懂這包資料**（key／value、`{ }` 是一筆、`[ ]` 是一串，並切清楚「這個 key 跟 API 金鑰無關」）+ 真的打 YouBike 即時資訊，做出一頁可上傳的即時看板                                                                                                                                                                                                                                                        |
...
| 10 · EXE 執行檔          | **不上網的那一種「給別人用」** | 四題「做網頁還是做 EXE」的判斷 → 打包示意 ＋ **防毒誤判**（機關現場第一道牆）→ **怎麼發新版本**：版本號 `v1.4.2` 怎麼看、changelog 六種分類（新增／變更／修正／移除／即將移除／資安）、三組「寫給程式看 vs 寫給使用者看」的改寫對照、可複製範本，最後帶出「網頁改一次所有人就是新的，EXE 的更新要自己扛」 |
