...
| 關卡                         | 主題                                             | 互動設計重點                                                                                                                                                                                                                                                                                                                                      |
| ---------------------------- | ------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1 · 上線的方法有哪些？       | **全景**：AI 工具的分享連結 → 機關正式系統       | 從學員已經有的經驗切入：**Claude Artifacts / Gemini Canvas / ChatGPT Sites** 三張卡點開看**入口在哪**（「左側選單 › Artifacts」這種點擊路徑，可另外放介面截圖，見 `public/images/README.md`）與「它幫你做了什麼」→ 歸納出部署就是三件事（檔案放別的地方、機器一直開著、給你網址）→ 四個市政情境判斷「分享連結夠不夠用」→ 依「要自己顧多少」排序的全景清單，每項可跳到對應關卡 |
...
| 3 · **GitHub Pages**（旗艦） | 免費、純滑鼠部署靜態網站                         | 概念測驗 → **站內模擬 GitHub 介面**點一遍 → 去真的 GitHub 做並**貼網址驗證**（範本是一頁 **Portaly 風格的電子名片**，主角是臺北市吉祥物熊讚——頭像、聯絡方式 icon、連結按鈕，RWD、單一檔案無外部資源；學生做完不用刪 repo，改成自己的就能一直用。另附四段可展開的延伸主題：**「先看這 6 個雷」**（首頁檔名、大小寫、絕對路徑、中文檔名、金鑰／`.env`、需不需要後端）、**「🔑 金鑰、`.env` 與 `.gitignore`」**（前端藏不住金鑰、外洩要撤銷重發而不是刪那行、四個檔案能不能上傳的判斷、可複製的 `.gitignore`）、**「🖼️ 想放自己的照片或圖片？」**（三步驟 + 四個 `<img src>` 寫法的破圖判斷）、**「📄 幫你的 repo 寫一份 README」**（README = 交接文件、五件必寫的事、可複製範本、Markdown 一句話）） |
| 4 · 這個我可以自己做嗎？     | 界線判斷                                         | 認五條紅線 + 6 題市政情境判斷（市民講座公告、名冊誤傳公開 repo、1999 陳情、跨局處系統…）（可自己做／先問資訊單位／不該這樣做）                                                                                                                                                                                                                    |
| 5 · **Netlify / Cloudflare Pages** | **自動建置上線**（同一類，但平台幫你做更多） | 手動上傳 vs **推上去就自動建置**的流程對照（順帶解釋「build 是什麼」，並指出這個教學網站本身就是 build 出來的）→ 三家比一比（會不會幫你 build／免費額度／能不能收表單／什麼時候選它），帶出「**檔案是你的，被綁住的只是網址**」→ 把第 3 關那個 repo 再接上一家、拿到第二個網址並貼上驗證 ＋ **「不用後端也能收表單」**（Netlify Forms）與隨之而來的三題界線判斷 |
| 6 · API 基礎                 | 前後端怎麼對話                                   | 餐廳點餐比喻 + 站內 API 測試器（送出一次，看回來的 JSON；點出「這支 API 一次給你全部，要篩要自己篩」）+ **怎麼看懂這包資料**（key／value、`{ }` 是一筆、`[ ]` 是一串，並切清楚「這個 key 跟 API 金鑰無關」）+ 真的打 YouBike 即時資訊，做出一頁可上傳的即時看板                                                                                                                                                                                                                                                        |
| 7 · GAS + API 推送           | 市民向市政信箱陳情 → 自動通知承辦科室            | 設定訊息/觸發 → 看封包送達模擬群組 + 可複製的 GAS 程式碼                                                                                                                                                                                                                                                                                          |
| 8 · AI 原生 + Hugging Face   | 把 AI 應用一鍵部署                               | 模擬建立 Space → 可互動的「市政信箱意見情緒分析」Demo                                                                                                                                                                                                                                                                                             |
| 9 · **Firebase** | **資料庫與登入**（界線最吃緊的一關） | 一個開關看出「它不是沒有後端，是後端別人幫你寫好了」，並解掉一個矛盾：**Firebase 的 `apiKey` 是門牌不是鑰匙**，公開沒關係，真正的門鎖是安全規則 → **安全規則模擬器**：三種規則 × 三種來訪者，看誰進得來（`if true`／只檢查登入／比對資料主人），配上真實數據（抽樣 950 個開源專案，近 1/4 資料庫全世界可讀）→ 四題界線判斷 ＋ 上線前五項檢查清單 |

> **為什麼「Firebase」排在納管前的最後一關**：它是第一次讓學員一個人就能架起一個真的系統
> （有資料庫、有登入）—— 也是界線最容易被跨過去的時候。放在「交給資訊單位納管」那道分界線
> 前面，剛好收尾整段「你自己做得到的範圍」。

> **為什麼「Netlify / Cloudflare Pages」排在界線之後**：這一關會教「不用後端也能收表單」——
> 聽起來很誘人，但表單資料會存在境外平台。先學完界線，學生才判斷得出來什麼能收、什麼不能收。
...
| 關卡                     | 主題                     | 互動設計重點                                                                      |
| ------------------------ | ------------------------ | --------------------------------------------------------------------------------- |
| 10 · Windows / Linux 自架 | **內網自用 vs 對外**     | 三個開關 + 分別測「同仁從市府內網連」與「市民從外網連」，帶出對外需資安評估與核准 |
| 11 · Docker              | 把 App 和環境一起打包    | 選擇是否打包環境 → 在別台機器 run，體會可攜性                                     |
| 12 · EXE 執行檔          | **不上網的那一種「給別人用」** | 四題「做網頁還是做 EXE」的判斷 → 打包示意 ＋ **防毒誤判**（機關現場第一道牆）→ **怎麼發新版本**：版本號 `v1.4.2` 怎麼看、changelog 六種分類（新增／變更／修正／移除／即將移除／資安）、三組「寫給程式看 vs 寫給使用者看」的改寫對照、可複製範本，最後帶出「網頁改一次所有人就是新的，EXE 的更新要自己扛」 |
...
關卡頁另有麵包屑（第 N / 12 關、所屬階段）與底部的上一關／下一關。
...
- **📇 名詞小教室**（`#/terms`）：12 張概念卡（快取、佇列、**Worker**、CDN、冪等、日誌監控、環境、備份、Rollback、登入vs權限、HTTPS、CORS 跨來源限制），其中**快取、冪等、佇列**有小互動。
  > Worker 卡的重點是**一個字三個意思**的區辨：佇列的背景工人／Cloudflare Workers（產品名）／瀏覽器的 Service Worker —— 三者毫無關係，只是同名。
  > 佇列原本綁在第 10 關（那時叫「EXE / Queue」），但它跟 EXE 是兩件事 —— 已連同模擬器搬到這張卡。
  > 名詞是**查閱型**內容，所以放在這裡而不是做成關卡：要加名詞就加卡片，不要為它開新的一關。
...
src/
  ...
  levels/                # 12 關各自一個檔案
    ...
    HostingLevel.jsx      FirebaseLevel.jsx
...
`tests/e2e.mjs` 會把 **12 關全部走完一遍**（含每關的互動、測驗與過關），並檢查「回上一步」與進度是否到 100%。用來確認改動沒有弄壞任何一關。
