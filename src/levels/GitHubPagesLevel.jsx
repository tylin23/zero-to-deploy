...
function SimStep({ onNext }) {
  ...
  return (
    <div className="space-y-4">
      <span className="uppercase tracking-[2.5px] text-xs font-extrabold text-accent">步驟 2 / 3 · 先練一次</span>
      <h2 className="text-2xl font-bold text-ink">在「模擬的 GitHub」點一遍</h2>
      <p className="text-muted text-sm">下面是模仿真的 GitHub 畫面。跟著發亮的按鈕點 —— 等一下去真的網站就會很眼熟。</p>
      ...
      <button type="button" className="btn btn-primary" onClick={onNext}>我練會了，換我真的做一次 →</button>
    </div>
  );
}

const Tip = ({ children }) => (
  <div className="inline-flex items-center gap-1.5 text-[12.5px] font-extrabold text-accentDark bg-accentSoft py-1 px-3 rounded-full mb-2.5">{children}</div>
);
  ...
  return (
    <Browser url="github.com">
      <div className="flex items-center gap-2.5 pb-3.5 border-b border-line mb-4">
        ...
        <button type="button" className="gh-btn gh-btn-green animate-pulseRing" onClick={onNext}>＋ New</button>
      </div>
  ...
  return (
    <Browser url="github.com/new">
      ...
      <label className="flex gap-2.5 items-start py-2.5 px-3 border-2 rounded-xl mb-2 border-primary bg-primarySoft">
        ...
        <div><b>Public</b><small className="block text-muted font-normal">公開 —— 這樣別人才連得到你的網站（要用 Pages 通常選這個）</small></div>
      </label>
      <Tip>👉 確認選了 Public，然後按綠色「Create repository」</Tip>
      <div><button type="button" className="gh-btn gh-btn-green animate-pulseRing" onClick={onNext}>Create repository</button></div>
    </Browser>
  );
}
  ...
  return (
    <Browser url={`github.com/你/${REPO}`}>
      ...
      <div
        ...
        onDrop={(e) => { e.preventDefault(); fill(); }}
        onClick={() => { if (picked) fill(); }}
...
function SimPages({ onNext }) {
  return (
    <Browser url={`github.com/你/${REPO}/settings/pages`}>
      <div className="flex gap-2 items-center mb-3"><span className="font-extrabold">⚙️ Settings</span><span className="text-muted text-sm">›  Pages</span></div>
      ...
      <div className="mb-3.5">
        ...
        <select className="gh-input"><option>main</option><option>None</option></select>
      </div>
      <Tip>👉 分支選 main，按 Save</Tip>
      <button type="button" className="gh-btn gh-btn-green animate-pulseRing" onClick={onNext}>Save</button>
    </Browser>
  );
}
...
function SimLive() {
  return (
    <Browser url={`github.com/你/${REPO}/settings/pages`}>
      <div className="callout" style={{ borderLeftColor: "#2da44e", background: "var(--success-soft)", color: "var(--success)" }}>
        <b>✅ Your site is live!  </b>網站已上線：<br />
        <code className="font-mono text-sm bg-surface2 px-1.5 py-0.5 rounded border border-line">https://你的帳號.github.io/my-first-site/</code>
      </div>
...
function RealStep({ onFinish }) {
  ...
  const validate = () => {
    ...
    if (ok) { setMsg({ ok: true, text: "網址格式正確，看起來就是 GitHub Pages！🎉" }); setTimeout(() => onFinish(val), 700); }
  ...
  return (
    <div className="space-y-4">
      <span className="uppercase tracking-[2.5px] text-xs font-extrabold text-accent">步驟 3 / 3 · 真的動手</span>
      <h2 className="text-2xl font-bold text-ink">換你在真的 GitHub 上部署 🚀</h2>
      <div className="callout">跟著清單一步步做。每做完一項就打勾。需要一個檔案的話，先<b className="text-ink">下載我們準備好的 index.html</b> 去上傳就好。</div>

      <button type="button" className="btn btn-accent" onClick={downloadStarter}>⬇ 下載範本 index.html</button>

      <ul className="list-none p-0 m-0 grid gap-2.5">
        {ITEMS.map((it, i) => (
          <li key={i} className={`flex gap-3 items-start py-3.5 px-4 border-2 rounded-[14px] transition-all ${checked[i] ? "border-mint bg-successSoft" : "border-line bg-surface"}`}>
            <input type="checkbox" checked={checked[i]} onChange={(e) => setChecked((c) => c.map((v, j) => (j === i ? e.target.checked : v)))}
              className="w-5.5 h-5.5 mt-0.5 shrink-0 cursor-pointer" style={{ accentColor: "var(--mint)", width: 22, height: 22 }} />
      ...
      {allChecked && (
        <div>
          ...
          <div className="flex gap-2.5 flex-wrap mt-2">
            <input className="gh-input flex-1 min-w-[200px]" type="url" placeholder="https://你的帳號.github.io/..."
              value={url} onChange={(e) => setUrl(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") validate(); }} />
            <button type="button" className="btn btn-primary" onClick={validate}>驗證我的網站 ✅</button>
          </div>
          {msg && <div className={`mt-2.5 text-sm font-bold ${msg.ok ? "text-success" : "text-danger"}`}>{msg.text}</div>}
          <p className="text-muted text-sm mt-2.5">（還沒真的做完也沒關係 —— 之後回來貼上網址就能拿到徽章。）</p>
        </div>
      )}
    </div>
  );
