    ...
    <div>
      ...
      >
        <svg
          ...
          viewBox={`0 0 ${layout.W} ${layout.height}`}
        ...
        {layout.dividerY != null && (
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <span
                className="px-3.5 py-1.5 rounded-full text-xs font-extrabold border-2 whitespace-nowrap"
                style={{
        ...
        {mapOrder.map((lv, i) => (
          <TrailNode
            ...
            index={i}
            pos={layout.pts[i]}
            done={isComplete(lv.id)}
            current={lv.id === currentId}
            navigate={navigate}
          />
        ))}
        ...
        <div
          ...
          style={{ left: layout.finish.x, top: layout.finish.y }}
        >
          <div className="text-[46px]" style={{ filter: "drop-shadow(0 4px 4px rgba(0,0,0,.15))" }}>
            🏆
          </div>
          ...
          <div className="flex gap-3.5 flex-wrap">
            {badgeList.map((b) => (
              <div
                ...
                style={{
                  background: "linear-gradient(150deg, var(--accent-soft), var(--primary-soft))",
...
function TrailNode({ lv, index, pos, done, current, navigate }) {
  ...
  if (locked)
    circle = {
      ...
      boxShadow: "0 8px 0 var(--border), var(--shadow-sm)",
      filter: "grayscale(.5)",
      opacity: 0.8,
      cursor: "not-allowed",
    };
    ...
    circle = {
      ...
      boxShadow: "0 8px 0 var(--mint-dark), var(--shadow-sm)",
    };
  else if (current)
    circle = {
      background: "linear-gradient(160deg, var(--accent), var(--accent-dark))",
      boxShadow: "0 8px 0 var(--accent-dark), var(--shadow)",
    };
  else
    circle = {
      background: "linear-gradient(160deg, var(--surface), var(--primary-soft))",
      boxShadow: "0 8px 0 color-mix(in srgb, var(--primary) 30%, var(--border)), var(--shadow-sm)",
    };
  ...
  return (
    ...
    >
      {/* 編號角標 */}
      <span
        className="absolute -top-2 -left-2 w-7 h-7 rounded-full border-2 text-xs font-extrabold grid place-items-center z-[2]"
        style={{ ...num, boxShadow: "var(--shadow-sm)" }}
      >
        {index + 1}
      </span>
      ...
      {done && (
        <span
          className="absolute -top-5 left-1/2 -translate-x-1/2 text-[15px] tracking-[-2px] whitespace-nowrap"
          style={{ textShadow: "0 2px 2px rgba(0,0,0,.15)" }}
        >
          ⭐⭐⭐
        </span>
      )}
      ...
      <button
        ...
        onClick={() => {
          if (!locked) navigate("#/level/" + lv.id);
        }}
      ...
      >
        <span
          ...
          style={{ filter: "drop-shadow(0 2px 2px rgba(0,0,0,.12))", opacity: locked ? 0.35 : 1 }}
        >
          {lv.emoji}
        </span>
      ...
      <div className="absolute top-[calc(100%+8px)] left-1/2 -translate-x-1/2 w-max text-center">
        <div
          className={`font-bold text-[12px] sm:text-sm leading-tight px-2.5 py-0.5 rounded-full whitespace-nowrap ${locked ? "text-muted" : "text-ink"}`}
