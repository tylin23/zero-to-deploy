...
const CASES = [
  ...
  {
    text: "手邊有一份講座報名名冊（含市民姓名、電話），想找個地方備份，就把 Excel 檔一起上傳到上一關那個 repo。",
    answer: "no",
    why: "repo 設成 Public，裡面「每一個檔案」都會公開，不是只有網頁那一頁 —— 名冊一傳上去，民眾個資就攤在網路上了。而且 GitHub 會留版本紀錄，事後刪掉也可能已經被搜尋引擎索引、被第三方存檔。備份請用機關既有的空間，別拿公開的 repo 當雲端硬碟。（想線上收報名資料的正確做法，第 7 關會教）",
  },
  {
    text: "把研考會已經公開的「陳情案件分類統計」，做成一張內部看板網頁給科室同仁參考。",
    answer: "self",
    why: "這份開放資料只有案件類別、受理機關和日期，沒有陳情人的姓名電話，也不是給市民用的正式服務 —— 屬於可以自己做的範圍。（第 6 關就會教你怎麼讓這張看板自動去抓最新資料）",
  },
  {
    text: "把 1999 市民熱線的陳情原文（含姓名、電話）貼進網路上的 AI 服務，請它幫你分類。",
    answer: "no",
    why: "兩條紅線同時踩到：一是個資，二是交給機關以外的第三方公司 —— 這類 AI 服務多半在國外，貼進去的文字就離開機關了。真要做，必須先把姓名電話等資訊拿掉，並依機關個資與資安規範取得核准。（第 8 關會實際做一次，也會再提醒這件事）",
  },
  {
  ...
  },
  {
    ...
    answer: "self",
    why: "通知只帶案號、不帶姓名電話，屬於科室內部的流程自動化，可以自己做。（第 7 關會實際做一次；到時候要注意，那組「發通知用的網址」等同鑰匙，別外流）",
  },
];
